...
const fetchListings = (searchQuery = "") => {
 ...
 <div class="listing-card">
    <h3 class="listing-title">${listing.itemName}</h3>
    <p class="listing-price"><strong>Price:</strong> $${listing.itemPrice.toFixed(2)}</p>
    <p class="listing-condition"><strong>Condition:</strong> ${listing.condition}</p>
    <p class="listing-description">${listing.itemDescription}</p>
    <p class="listing-user"><strong>Listed by:</strong> ${username || "Unknown User"}</p>
    ${listing.customAttribute ? `<p class="listing-category"><strong>Category:</strong> ${listing.customAttribute}</p>` : ""}
    <p class="listing-views"><strong>Views:</strong> ${listing.viewCount}</p>
    <img class="listing-image" src="${listing.imageUrl}" alt="${listing.itemName}" />

    <div class="listing-actions">
        <button class="btn btn-details" onclick="viewDetails('${doc.id}')">View Details</button>
        <button class="btn btn-offer" onclick="offerPrice('${doc.id}', '${listing.userId}')">Make an Offer</button>
        <button class="btn btn-buy" onclick="buyItem('${doc.id}', '${listing.userId}', '${listing.itemPrice}')">Buy</button>
    </div>
</div>
